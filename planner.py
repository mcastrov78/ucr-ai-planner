...
def plan(domain, problem, useheuristic=True):
    ...
    def heuristic(state, next_action):
        ...
        f = state
        # relaxed plan will layers, each with and action layer and a propositions layer
        relaxed_plan = [[[], f]]
        ...
        while not isgoal(f):
            ...
            new_world = expressions.World(next_layer_atoms, f.world.sets)
            f = graph.ExpressionNode(new_world, f.actions, f.preceding_action)
            # add layer to relaxed plan
    ...
    def get_first_goal_levels(rpg, goal, first_goal_levels):
        ...
        # handle special case when goal is not a conjunction
        ...
        for sub_goal in first_goal_levels[layer]:
            ...
            for layer_index in range(len(rpg)):
                for action in rpg[layer_index][0]:
                    previous_state = rpg[layer_index - 1][1]
                    next_state = action.target
                    if not previous_state.world.models(sub_goal) and next_state.world.models(sub_goal):
                        if level in action_levels:
                            action_levels[level].append(action)
                        else:
