...
def plan(domain, problem, useheuristic=True):
    ...
    def heuristic(state, next_action):
        relaxed_plan = []
        # initial state is a following neighbor state that A* needs to evaluate
        f = copy.deepcopy(state)
        relaxed_plan.append([[], f])
        # where in the planning graph we are, extend one action layer and proposition layer at a time
        t = 0
        while not isgoal(f):
            # increase layer index and compute next layer
            t += 1
            # next action layer: get "relaxed" neighbors whose Add List has a real effect and ignoring Delete Lists
            next_layer_actions = f.get_relaxed_neighbors()
            # next atom layer: start with atoms in current layer and add new atoms generated by each new action
            next_layer_atoms = f.world.atoms
            print("\t\tRPG next actions (t=%s): %s" % (t, ", ".join(neighbor.name for neighbor in next_layer_actions)))
            for next_action in next_layer_actions:
                next_layer_atoms = next_layer_atoms.union(next_action.target.world.atoms)

            # stop if all next layer atoms are included on the previous one, otherwise continue in the loop
            if f.world.atoms.issuperset(next_layer_atoms):
                break

            f.world.atoms = next_layer_atoms
            relaxed_plan.append([next_layer_actions, f])

        plan_size = extract_plan_size(relaxed_plan)
        print("\t\t* RPG world.atoms (%s): %s" % (plan_size, f.world.atoms))
        return plan_size
    ...
    def extract_plan_size(rpg):
        ...
        #print("\t\tRGP - Final state %s: " % final_state.world)
        #print("\t\tRGP - Is goal?: %s" % isgoal(final_state))

        # if the state (world) in final proposition layer does not contain the goal
        if not isgoal(final_state):
            return 1000

        # find the first layer where each sub-goal appears for the first time on the relaxed planning graph
        first_goal_levels = get_first_goal_levels(rpg, goal)
        # backtrack starting on the last proposition layer we need to consider
        for i in range(len(first_goal_levels) - 1, 0, -1):
            print("i: %s" % i)
            get_first_action_levels(rpg, goal)

        return len(rpg) - 1

    def get_first_goal_levels(rpg, goal):
        """Find the first layer where each sub-goal appears for the first time on the relaxed planning graph"""
        goal_levels = []

        # for each sub-goal in goal
        for sub_goal in goal[1:]:
            level = 0
            # for each layer in relaxed planning graph
            for layer in rpg:
                # check if the state (world) in this layer models this sub-goal
                if layer[1].world.models(expressions.make_expression(sub_goal)):
                    if len(goal_levels) >= level:
                        goal_levels.insert(level, [sub_goal])
                    else:
                        goal_levels[level].extend(sub_goal)
                    break
                level += 1

        print("GOAL LEVELS: %s" % goal_levels)
        return goal_levels

    def get_first_action_levels(rpg, goal):
        """Find the first layer where the action that causes each sub-goal appears for the first time on the relaxed planning graph"""
        action_levels = []

        for sub_goal in goal[1:]:
            level = 0
            print("Subgoal: %s" % sub_goal)
            for layer in rpg:
                for action in layer[0]:
                    print("Action (level %s): %s" % (level, action.name))
                level += 1
